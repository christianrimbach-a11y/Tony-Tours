...
// ── GPX Route 2 Slideshow (unter 243-Kurven-Block) ───────────────
(function () {
  let current = 0;
  ...
  function update(index) {
    const slides = document.querySelectorAll("#gpx-route2-slideshow .gpx-offer2-slide");
    const dots = document.querySelectorAll("#gpx-route2-dots .gpx-offer2-dot");
    if (!slides.length) return;
  ...
  window.gpxRoute2GoTo = function (index) {
    current = index - 1;
    update(index);
    if (timerId) {
